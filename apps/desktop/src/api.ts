...
export const api = {
  ...
  updateTask: (id: string, input: Partial<CreateTaskInput>) => request<TaskDto>(`/tasks/${id}`, { method: "PATCH", body: JSON.stringify(input) }),
  ...
  updateContextItem: (id: string, input: Partial<CreateContextItemInput>) =>
  ...
  updateContextPack: (id: string, input: Partial<CreateContextPackInput>) =>
  ...
  getAgentRuns: (projectId: string) => request<Array<AgentRunDto & { contextDiff: unknown }>>(`/agent-runs?projectId=${projectId}`),
